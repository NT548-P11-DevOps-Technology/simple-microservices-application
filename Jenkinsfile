pipeline {
    ...
    stages {
        ...
        stage('Deploy to Kubernetes') {
            steps {
                withKubeConfig(caCertificate: '', clusterName: 'kubernetes', contextName: '', credentialsId: 'k8s-token', namespace: 'default', restrictKubeConfigAccess: false, serverUrl: 'http://13.212.36.112:6443/') {
